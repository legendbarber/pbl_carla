...
class LidarPublisher:
    ...
    def handle(self, carla_lidar_measurement):
        ...
        # Convert by flipping y so RViz and planners see correct left/right.
        pts = np.frombuffer(lidar_bytes, dtype=np.float32)
        if pts.size % 4 == 0:
            pts = pts.reshape((-1, 4)).copy()
            pts[:, 1] *= -1.0
            lidar_bytes = pts.astype(np.float32, copy=False).tobytes()
...
class SemanticLidarPublisher:
    ...
    def handle(self, semantic_lidar_measurement):
        ...
        # CARLA->ROS: flip y (right->left). Keep other fields intact.
        dtype = np.dtype([('x','<f4'),('y','<f4'),('z','<f4'),('cos_incidence','<f4'),('obj_idx','<u4'),('obj_tag','<u4')])
        pts = np.frombuffer(data, dtype=dtype).copy()
        pts['y'] *= -1.0
        data = pts.tobytes()
        num_points = pts.shape[0]
